...
def _create_compose(location: str, cids: List[int], host_port: Tuple[str, int], clients_per_round: int, max_rounds: int, mem_fraction: float, epochs: int, batch_size: int):
    ...
    for c in cids:
        ...
        services[service_name] = {
            ...
            "command": f"""python app-client.py --mongo_uri "mongodb://0.0.0.0:27017/" --loc="{location}" --data_path "dataset/pecanstreet/15min/{location}/train/" --test_path "dataset/pecanstreet/15min/{location}/test/"  --host fl-server --filter_bs {c} --epochs {epochs} --batch_size {batch_size} --num_workers 0""",
            "environment": [
                "NVIDIA_VISIBLE_DEVICES=all",
