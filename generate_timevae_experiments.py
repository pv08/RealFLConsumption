"""Gera o docker-compose da matriz de experimentos do TimeVAE generativo (braços x sementes).

Cada serviço é uma execução de `eval_timevae.py` com um `--arm` e uma `--seed`; os serviços são
encadeados por `service_completed_successfully` para não disputarem a GPU. Espelha o papel do
`generate_simulation.py` para a simulação FL.

    python generate_timevae_experiments.py --seeds 0 1 2
    docker compose -f docker-compose.timevae.yml up --build
...
def build_service(arm, seed, args, previous, n_seeds):
    tag = run_tag(arm, seed, n_seeds, args.gen_holdout_days)
    service = {
        "build": {"context": ".", "dockerfile": "Dockerfile.timevae"},
        "image": "timevae-eval-img",
        "container_name": f"timevae_{arm.lower()}_s{seed}",
        "runtime": "nvidia",
        "volumes": ["./etc:/app/etc", "./dataset:/app/dataset:ro"],
        "environment": [
...
def main():
    ...
    parser.add_argument("--filter_bs", type=int, default=661)
    ...
    parser.add_argument("--out", type=str, default="docker-compose.timevae.yml")
    ...
    # Ordem: braço externo, semente interna — assim as réplicas de um braço saem juntas e um
    # braço completo já é comparável mesmo se a fila for interrompida no meio.
    services, previous = {}, None
    for arm in args.arms:
        for seed in args.seeds:
            name = f"{arm.lower()}-s{seed}"
            services[name] = build_service(arm, seed, args, previous, len(args.seeds))
            previous = name

    header = (
        "# GERADO POR generate_timevae_experiments.py — NÃO EDITE À MÃO.\n"
        f"# Braços: {', '.join(args.arms)} | sementes: {args.seeds} | "
        f"{len(services)} execuções em série.\n"
        "#\n"
        "#   docker compose -f %s up --build\n"
        "#\n"
        "# Rodam em série (service_completed_successfully) porque dividem uma GPU. Para uma\n"
        "# execução isolada: docker compose -f %s up --no-deps <serviço>\n"
        "#\n"
    ...
    log(INFO, f"{len(services)} serviços ({len(args.arms)} braços x {len(args.seeds)} sementes) "
              f"escritos em {args.out}")
