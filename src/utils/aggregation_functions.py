...
def simple_aggregate(results: List[Tuple[List[np.ndarray], int]]) -> np.ndarray:
    """Compute a simple avg"""
    weights = [
        [layer for layer in weights] for weights, _ in results
    ]

    weights_prime: np.ndarray = [
        reduce(np.add, layer_updated) / len(weights) for layer_updated in zip(*weights)
    ]
...
def median_aggregate(results: List[Tuple[List[np.ndarray], int]]) -> np.ndarray:
    ...
    weights_prime: np.ndarray = [
        np.median(layer_updated, axis=0) for layer_updated in zip(*weights)
    ]
    return weights_prime
...
def fedavg_aggregate(results: List[Tuple[List[np.ndarray], int]]) -> np.ndarray:
    ...
    weights_prime: np.ndarray = [
        reduce(np.add, layer_updated) / num_examples_total for layer_updated in zip(*weighted_weights)
    ]
    return weights_prime

def fednova_aggregate(results: List[Tuple[List[np.ndarray], int]], previous_model: List[np.ndarray], rho: float=0.0) -> List[np.ndarray]:
    """Compute weighted avg according to FedNova"""
